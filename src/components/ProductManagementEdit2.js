...
function ProductManagementEdit2(props) {
  ...
  useEffect(() => {
    const fetchData2 = async () => {
      try {
        const response = await axios.post(
          `${process.env.REACT_APP_APIENDPOINT}admin/product/select-brand/${productListItems[0]?.category_Id?._id}`
        );
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.post(
          `${process.env.REACT_APP_APIENDPOINT}admin/category/attribute/selectSubSubCategory/${productListItems[0]?.category_Id?._id}`
        );
  ...
  const handleOnSave = async (e) => {
    ...
    const res = id
      ? await updateProduct({ data, id }).then((res) => {
          Swal.fire({
  ...
  const handleOnSave1 = (e) => {
    ...
    axios
      .post(
        `${process.env.REACT_APP_APIENDPOINT}admin/product/new-varient/${id}`,
        data
      )
      .then((response) => {
